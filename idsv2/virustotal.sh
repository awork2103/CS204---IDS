...
for file in "$FILEOBJ_DIR"/*; do
    if [ -f "$file" ]; then  # Ensure it's a regular file not directory
        ...
        if [[ $VT_SCORE_INT -ge 10 ]]; then
            # echo "hash: $HASH, VT Score: $VT_SCORE" > ./virustotal-files/$(date +%Y%m%d%H%M%S)/malicious_file_upload.txt
            ./sendNotif.sh "$FILEOBJ_DIR/ip_file_map.txt" $SENDER_IP "Malicious file upload. Hash: $HASH, VT Score: $VT_SCORE"
            # rm ./virustotal-files/$(date +%Y%m%d%H%M%S)/malicious_file_upload.txt 
        fi
        # rm $file
    fi
done
